...
**Excluded from boundary**: Parent DNS zone (separate AWS account), end-user workstations, AWS management plane (covered by AWS FedRAMP P-ATO).
...
Internet
   |
   v
[Route53 DNS] --> [ALB + WAF (Public Subnets, 2 AZs)]
                          |
                     [TLS 1.3 termination]
                          |
...
|---|---|---|---|---|---|
| 1 | Route53 (parent zone) | Outbound (NS delegation) | DNS | NS records delegating subdomain | Parent zone is in a separate AWS account; not within authorization boundary |
